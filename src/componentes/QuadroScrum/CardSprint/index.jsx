...
export default function CardSprint({ data, index, listIndex, cliente, contrato }) {
  ...
  return (
    <>
      <div 
        ref={ref} 
        className={`relative mb-2.5 text-black p-[15px] shadow-[0_1px_4px_0_rgba(192,208,230,0.8)] border-t-[20px] border-t-[rgba(230,236,245,0.4)] transition-[width] duration-[600ms] ${
          isDragging 
            ? 'border-2 border-dashed border-black/20 pt-[31px] rounded-none bg-transparent shadow-none cursor-grabbing [&_p]:opacity-0 [&_img]:opacity-0 [&_span]:opacity-0 [&_h3]:opacity-0' 
            : 'bg-white rounded-[5px]'
        }`}
      >
        <header className="relative -top-2.5 h-10">
        ...
        <p className="font-medium text-justify whitespace-pre-wrap w-full">{data.content}</p>
        {
          backlogs.length > 0 ?
            (
        ...
        <div className="flex flex-row flex-wrap mt-4">
          {
            files?.map((file, idx) =>
              <div key={idx} className="inline-flex rounded-[2px] border border-[#eaeaea] mb-2 mr-2 w-[80px] h-[80px] p-1 cursor-pointer" onClick={() => abrirPreview(file)}>
                <div className="flex min-w-0 overflow-hidden">
                  <img className="block w-auto h-full" src={`data:image/jpeg;base64,${file.base64}`} alt={file.nome} />
                </div>
              </div>
            )
          }
