...
function Card({ cliente, contrato, projeto_id, ocorrencia, atendente = 0, nomeAtendente, cod_ocorrencia, data, showActions = true, children }) {
  ...
  function AddCheckbox(item) {
    ...
    return (
      <div className="checkbox">
        ...
        {!isChecked ? item : <span className="line-through">{item}</span>}
      </div>
    );
  }
  ...
  return fechada ? null
    : (
      <>
        <div className={`w-full max-w-4xl p-6 rounded-xl shadow-md border border-slate-200 flex flex-col gap-4 transition-transform hover:-translate-y-1 ${funAtendente !== 0 ? "bg-white" : "bg-green-100"}`}>
          <div className="flex flex-col gap-4 w-full">
            <div className="flex justify-between items-start border-b border-slate-200 pb-4">
              <h3 className="font-bold text-lg text-slate-800">{clienteAtual}</h3>
              <div className="flex flex-col items-end gap-1">
                <span className="text-sm text-slate-500">{new Date(data).toLocaleDateString()}</span>
                {funAtendente ? (
                  <span className="bg-slate-100 px-3 py-1 rounded-full text-xs font-semibold text-slate-700 shadow-sm">{nome_atendente}</span>
                ) : (
                  <span className="bg-red-100 px-3 py-1 rounded-full text-xs font-semibold text-red-700 shadow-sm">Nao atendida</span>
                )}
              </div>
            </div>
            {contaCheckBox()}
            {!showActions ? (
              <div className="w-full h-2 rounded-full bg-green-200 overflow-hidden">
                <div className="bg-green-600 h-full flex items-center justify-center text-[10px] text-white" style={{ width: `${((num_tarefas_realizadas / num_tarefas) * 100) * -1}%` }}>
                  {parseFloat(((num_tarefas_realizadas / num_tarefas) * 100) * -1).toFixed(0)}%
                </div>
              </div>
            ) : null}
          ...
          {showActions ? (
            <div className="flex flex-wrap justify-end gap-2 bg-slate-50 p-3 rounded-lg border border-slate-100 mt-2">
              <Button Icon={MdScanner} click={Atender} nome={"Atender"} color={"#F0F0F2"} corTexto={"black"} borderRadius={'8px'} />
              <Button Icon={MdEdit} click={abrirModalTrocarCliente} nome={"Trocar Cliente"} color={"#3498db"} corTexto={"white"} borderRadius={'8px'} />
              <Button Icon={MdCancel} click={finalizar} nome={"Fechar"} color={"#733130"} corTexto={"white"} borderRadius={'8px'} />
              <Link to={{ pathname: '/quadroScrum', state: { cliente: clienteAtual, projeto_id: projetoAtual, contrato: contratoAtual, ocorrencia: cod_ocorrencia } }} >
                <Button Icon={MdClose} nome={"Abrir Scrum"} color={"#7FA66D"} corTexto={"white"} borderRadius={'8px'} />
              </Link>
            </div>
          ) : children}
        </div>
